...
class AboutMethods extends KoanSuite {
  ...
  koan(
    """If a method does not have equal its return type is
      | considered to be `Unit` which is analogous to `void` in Java""") {
  ...
  koan(
    """If you want to have an = on the method, while
      | still explicitly returning Unit you can make the return type `Unit`,
  ...
  koan(
    """Once you have an =, it is understood
      | that there will be a return type and can be inferred""") {
    def foo(x: Int) = 3 + 4
  ...
  koan(
    """Of course if you wish to be explicit about the
      | return type, you can attach it at the end of the method""") {
  ...
  koan(
    """If you want to ensure a method is not only recursive
      | but _tail recursive_,
      | you can get help from the scala compiler to ensure
      | that it is indeed a
      | tail recursive call by
      | including scala.annotation.tailrec on the method.
      | When methods are properly tail recursive. The
      | Scala compiler will optimize the code from stack
      | recursion into a loop at compile time""") {
  ...
  koan(
    """In scala, methods can be placed inside in methods!
      | This comes useful for
      | recursion where accumulator helper methods can
      | be placed inside the outer
      | method, or you just want to place one method in
      | another for design reasons""") {
    def factorial(i: BigInt): BigInt = {
      @tailrec
      def fact(i: BigInt, accumulator: BigInt): BigInt = {
        if (i <= 1)
  ...
  koan(
    """Remember you can have strange characters in values
      | and variables as long as they're
      | after an underscore, well you can do the
      | same in methods""") {
  ...
  koan(
    """If you also remember you can add reserved words or
      | words with space to any value or variable?
      | Same applies for methods. Although please note
      | that this is uncommon unless you really are into
      | internal DSLs""") {
  ...
  koan(
    """Convention (not required for the compiler) states that
      | if you a call a method without any parameters that
      | returns a Unit, invoke that method with empty
      | parenthesis, other than leave the parenthesis out""") {
  ...
  koan(
    """Methods with colons are right-associative, that
      | means the object that a method is on will be on
      | the _right_ and the method parameter will be on
      | the _left_""") {
